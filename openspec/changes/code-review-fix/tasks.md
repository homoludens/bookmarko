...
- [x] 3.4 Update import status endpoints/handlers to require scope context and prevent cross-user leakage. `Complexity: Middle`
- [x] 3.5 Add tests covering startup config validation failures and concurrent import status isolation. `Complexity: Middle`
...
- [x] 4.2 Apply count-based ordering in SQL before pagination boundaries are computed. `Complexity: Middle`
- [x] 4.3 Validate query behavior against representative data and adjust indexes if needed. `Complexity: Middle`
- [x] 4.4 Add tests that verify global count ordering across paginated results. `Complexity: Low`
